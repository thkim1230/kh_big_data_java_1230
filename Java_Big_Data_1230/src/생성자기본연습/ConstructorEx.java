...
// 생성자 만드는 규칙
// 1. 생성자는 클래스 이름과 같아야함.
// 2. 반환 타입은 없고 접근제한자는 올 수 있으나 private 는 특별한 경우가 아니면 사용 안함.
// 3. 하나의 클래스는 여러개의 생성자를 가질 수 있으나 , 오버로딩 생성 규칙에 맞아야 함.
...
class Car{
    ...
    Car(String name,String year){
        this.name = name;
        this.year = year;
    } // 생성자가 다르려면 타입과 개수만 보기때문에 개수나 타입이 바뀌어야 함 (타입과 개수가 같으면 변수 이름이 달라도 의미없음)
