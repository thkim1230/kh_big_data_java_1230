package 문자열다루기;
public class StringType {
    public static void main(String[] args) {
        String a = "hello";
        String b = "Java";
        String c = "hello";
        // equals() 메소드 : 두개의 문자열이 동일한지 비교하여 결과값을 return (true,false).
        System.out.println(a.equals(b)); // a의 문자열과 b의 문자열을 비교
        System.out.println(a.equals(c)); // a 와 c를 비교
        System.out.println(a.equalsIgnoreCase("HELLO")); // 대소문자 무시하며 같은지 비교
        System.out.println(a == c); // 이건 문자열의 값을 비교하는게 아니고 문자열의 주소를 비교

        String d = " Hello Java";
        // indexOf() : 문자열에 특정 문자 혹은 문자열이 시작되는 인덱스(열 번호)를 리턴
        System.out.println(d.indexOf("Ja"));
        // contains() : 문자열에서 특정 문자열의 포함여부를 리턴(true , false)
        System.out.println(d.contains("hello"));// 대소문자 구분 함
        // charAt() : 문자열에서 특정 위치의 문자를 리턴 함
        System.out.println(d.charAt(1));
        // replaceAll() : 문자열 중 특정 문자열을 다른 문자열로 대체 할 때 사용
        System.out.println(d.replaceAll("Java","")); // 문자열을 대체하며 지울수도 있음
        // substring() : 문자열에서 특정 문자열을 뽑아 낼 때 사용
        System.out.println(d.substring(6));// 시작 인덱스 부터 끝까지 추출
        System.out.println(d.substring(1,4));// 시작 인덱스 부터 종료 인덱스 미만 까지 추출
        // toUpperCase / toLowerCase() : 문자열을 모두 대문자/소문자로 변경
        System.out.println(d.toUpperCase());
        System.out.println(d.toLowerCase());
        // trim() : 문자열의 앞 뒤에 있는 공백 제거
        System.out.println(d.trim());
        // 문자열 포매팅 : 문자열에 특정 문자를 삽입하는 방법 (삽입을 위해서는 서식이 필요)
        System.out.printf("자바 프로그램을 %s 합시다\n", "열심히");
        String newStr = "";
        int number = 10;
        String day = "three";
        newStr = String.format("I ate %d apples. so I was sick for %s days",number,day);
        System.out.println(newStr);

        String newStr2 = "I ate " + number + " apples. so I was sick for " + day + " days.";
        System.out.println(newStr2);
        // toCharArray() : 문자열을 문자 배열로 반환
        String name = "곰돌이사육사";
        char[] arrName = name.toCharArray();// 문자열을 문자로 구성된 배열로 만듬
        System.out.println(name);
        System.out.println(arrName);;
    }
}
